...
void setValues(){
 ...
 //Destination
  ...
  //VOLUME
  amp1.gain(map(volumeValue, 0, 1023, 0, 1000)/100.0f);

  //DYNAMIC
  for (int voice = 0; voice < NUM_VOICES; voice++) {
    for (int groupIndex = 0; groupIndex < 3; groupIndex++){
    waveforms[groupIndex][voice]->frequencyModulation(map(dynamicValue, 0, 1023, 0, 100)/100.0f);
    }
  }
  // Serial.println(map(dynamicValue, 0, 1023, 0, 100)/100.0f);
...
void handleEncoders(int currentPosition) {
  ...
  float detuneHz = 0;  // Declare detune variable outside switch to avoid scope crossing issue
  // Read all encoders
  for (int i = 0; i < NUM_ENCODERS; i++) {
    ...
    if (encCurrent[i] != encoderLast[i]) {
      ...
      switch (currentPosition) {
        ...
        case 9:  // Waveform type selection
          values[currentPosition][i] += adjustment;  // Increment or decrement the value
          values[currentPosition][i] = clampValue(values[currentPosition][i], 0, 100);  // Clamp to 0 - 100

          // Change waveform every 25 steps
          if (values[currentPosition][i] % 25 == 0) {
            ...
            switch (waveformIndex) {
              ...
              case 4:
                ...
                Serial.println("Waveform: REVERSE SAWTOOTH");
                break;
            }
